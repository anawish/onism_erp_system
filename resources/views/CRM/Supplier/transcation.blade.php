			...
			<table id="MasterTable" class="table table-striped table-bordered">
		        ...
		        <tbody>
		            ...
		            <tr class="autonumber">
		                ...
		                <td>$ {{number_format($datas->payment_made,2)}}</td>
		                <td></td>
		                <td>{{$client}}</td>
		                <td>{{$datas->payment_method}}</td>
		                <td>
		                	<a href="{{url('supplier/view',$datas->id)}}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i> View</a>

		                <!-- 	<a href="{{url('supplier/delete',$datas->id)}}" class="btn btn-danger btn-sm delete-object" onclick="return confirm('Are you sure you want to remove this data?')"><i class="fa fa-trash"></i></a> -->
		                </td>
		            </tr>
		            @endforeach
